 ...
 * @author jomsa
 */
public class EditProfessor extends javax.swing.JDialog {

    /**
     * Creates new form EditProfessor
     */
    public EditProfessor(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
        setResizable(false);
    ...
    public void updateProf(int profID, String[] prof_details, String gender, String dobString) {
        ...
        try {
            ...
            currentValues.put("professor_name", getCurrentValue(conn, "professor_details", "prof_name", profID));
            currentValues.put("prof_sex", getCurrentValue(conn, "professor_details", "prof_sex", profID));
            currentValues.put("prof_dob", getCurrentValue(conn, "professor_details", "prof_dob", profID));
            currentValues.put("prof_contact", getCurrentValue(conn, "professor_details", "prof_contact", profID));
            currentValues.put("prof_email", getCurrentValue(conn, "professor_details", "prof_email", profID));
            currentValues.put("address", getCurrentValue(conn, "professor_details", "address", profID));
            currentValues.put("dept", getCurrentValue(conn, "professor_details", "dept", profID));
            currentValues.put("spec", getCurrentValue(conn, "professor_details", "spec", profID));

            // Update professor_details
            updateTable(conn, "UPDATE professor_details SET prof_name = ?, prof_sex = ?, prof_dob = ?, prof_contact = ?, prof_email = ?, address = ?, dept = ?, spec = ? WHERE profID = ?",
                    new Object[]{
                            !prof_details[0].equals(currentValues.get("professor_name")) ? prof_details[0] : currentValues.get("professor_name"), // prof_name
                            gender, // prof_sex
                            !dobString.equals(currentValues.get("prof_dob")) ? dobString : currentValues.get("prof_dob"), // prof_dob
                            !prof_details[3].equals(currentValues.get("prof_contact")) ? prof_details[3] : currentValues.get("prof_contact"), // prof_contact
                            !prof_details[2].equals(currentValues.get("prof_email")) ? prof_details[2] : currentValues.get("prof_email"), // prof_email
                            !prof_details[4].equals(currentValues.get("address")) ? prof_details[4] : currentValues.get("address"), // address
                            !prof_details[5].equals(currentValues.get("dept")) ? prof_details[5] : currentValues.get("dept"), // dept
                            !prof_details[6].equals(currentValues.get("spec")) ? prof_details[6] : currentValues.get("spec"), // spec
                            profID // profID
                    });
            ...
            JOptionPane.showMessageDialog(null, "Error updating professor details.", "Error", JOptionPane.ERROR_MESSAGE);
        } finally {
            try {
                if (conn != null) {
    ...
    private String getCurrentValue(Connection conn, String tableName, String columnName, int profID) {
        String query = "SELECT " + columnName + " FROM " + tableName + " WHERE profID = ?";
    ...
    private void saveBttnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveBttnActionPerformed
        ...
        updateProf(profID, prof_details, gender, dobString);
        this.dispose();
